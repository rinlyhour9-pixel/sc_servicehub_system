...
@section('title', 'Technician sign in')
@section('subtitle', __('Technician sign in'))
@section('content')
    <h2 class="font-display text-2xl font-semibold text-ink-900 mb-5">{{ __('Technician Sign In') }}</h2>
    ...
    <p class="text-sm text-ink-900/60 mt-5 text-center">{{ __('Technician accounts are created by an administrator.') }}</p>
    <p class="text-sm text-ink-900/60 mt-1 text-center">
        <a href="{{ route('login') }}" class="text-rust font-medium">{{ __('Administrator sign in') }}</a>
    </p>
@endsection
